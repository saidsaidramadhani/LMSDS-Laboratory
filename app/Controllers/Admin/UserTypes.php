...
class UserTypes extends BaseController
{
	...
	{	
		...
		if ($this->request->getPost('show')=='Show') {
			...
			$data	=	$this->showrec($data);			
			echo view('admin/Add', $data);
			echo view('admin/modals/md_user_types');
	...
	public function ajax_update()
	{
		...
		$id						=	$this->request->getPost('user_type_id');
		$datas = array(
		...
		if (!$this->Common_model->isDuplicate('user_types',$fields,$value)) {			
			if ($this->Common_model->addRecord('user_types', $datas)) {
				session()->setFlashData('msg', 'user type added successfully');
				echo json_encode(array("status" => TRUE,"msg"=>"user types added successfully"));
			} else {
			...
			$where 		=	array('id'=>$id);
			if ($this->Common_model->updateRecord('user_types',$datas,$where)) {
				session()->setFlashData('msg', 'User types Changed successfully');
				echo json_encode(array("status" => TRUE,"msg"=>"User types Changed successfully"));
			} else {
	...
	function showrec($data) {
		...
		$_SESSION['ROUTE_NO'] ='user_types'; //the route configuration in config/routes.php for this controller		
		$data['headData'] = array('S/No','Type Name');
		$data['mapData'] = array('name'=>'L');
		...
		$sql		=	"SELECT * FROM user_types 
			";
		$data['addNew'] = array('do_user_types','id','delete_user_type');		
		//$query = $this->my_pay_fee_model->get_Records($where_arr);
		$query		=   $this->Common_model->run_query($sql);		
		if ($query) {
			$data['queryData'] = $this->Multi_model->showRecords($query, $action, $ref_tables);
			$data['editURI'] = array(site_url().'/mo_dept/user_types/index','id');
			$data['deleteURI'] = array(site_url().'user_types/delete','id');
	...
	public function delete($id)
	{
		...
		if ($this->Common_model->isDuplicate('user_types',$fields,$values)) {			
			$wheres			=	array('id'	=>	$id);							
			$aff_rows = $this->Common_model->deleteRecord('user_types','id', $id);
			if ($aff_rows) {
				$response=array("status" => TRUE,"msg"=>"User types deleted successfully");
			} else {
				$response=array("status" => FALSE,"msg"=>"User types could not be deleted");
			}
		} else {
				$response=array("status" => FALSE,"msg"=>"User types is not found");
		}
