...
class El {
	...
	var $label_suffix = '';
	var $field_prefix = "<div class='col-sm-12'>";				// for fieldset|text|password|upload|checkbox|radiobutton|image|submit|reset|textarea|button|captcha|recaptcha
	var $field_suffix = '</div>';		
	...
	var $_valid_atts = array(
		...
		'input' 	=> 'accept|accesskey|align|alt|checked|disabled|ismap|onmouseover|maxlength|name|onblur|onchange|onfocus|onselect|readonly|size|src|tabindex|type|usemap|value',
		'select' 	=> 'disabled|multiple|onblur|onchange|onfocus|size|tabindex', // 'name' not included, supplied with form helper function
		'textarea'	=> 'cols|rows|accesskey|disabled|name|onblur|onmouseover|onchange|onfocus|onselect|readonly|tabindex',
	...
	function _make_label($pos='')
	{
		if ($this->label_text)
		{
			...
			if ( $type != 'radio') { 
				if (isset($atts['tips'])) {
				$tips=$atts['tips'];
				//$this->label_text	  .='<a href="" data-toggle="popover" data-trigger="hover" data-content="'.$tips.'" data-placement="top" title="'.$tips.'"><img src="'.base_url().'images/help.gif" height="12" width="12" valign="middle" style="border:none"></a>';
				$this->label_text	  .='<a href="" data-toggle="tooltip" data-placement="top" title="'.$tips.'"><img src="'.base_url().'images/help.gif" height="12" width="12" valign="middle" style="border:none"></a>';
				}
			}
	...
	function date() 
	{
		...
		//$el .= "<a href='#' class='hintanchor' onMouseover="alert('A location must')"</a>";
	...
	function text() 
	{
		...
		$el .= $this->_make_label('after');
		$el .= $this->_make_error($this->name);
		//$el .= "<a href='#' class='hintanchor'</a>";
		//$el .= 'data-toggle="tooltip" data-placement="bottom" title="input with tooltip!!"';
		return $this->element_prefix.$el.$this->element_suffix;
	}
	...
	function select() 
	{
		...
		if ( ! empty($atts['data-live-search'])) $atts['data-live-search'] = 'true';
		$atts['data-live-search'] = 'true';		
...
class Form {
	...
	{
		//$tips 	= 	array('onmouseover'=>"showhint('ggggggggggggggg', this, event, '425px')");
		//foreach ($att as $keys => $values) 
		//	$tips[$keys]=$values; 
		//$atts	=	array($atts,$tips);
	...
	public function createBanner($msg, $colspan=1, $align='left') {

		$data_out = '<br><table><TR>';		
		$data_out .= '<div  class="bannercell" >'.$msg.'</div>';
		$data_out .= '</TR></table><br>';
	...
	function date($nameid, $label='', $rules='', $value='', $atts=array(),$tip='',$att=array()) 
	{
		if ($tip!='') {
			$tips 	= 	array('onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			foreach ($att as $keys => $values) 
				$tips[$keys]=$values; 
			$atts	=	array($atts,$tips);
		}	
	...
	function text($nameid, $label='', $rules='', $value='', $atts=array(),$tip='',$att=array()) 
	{
		if ($tip!='') {
			//$tips 	= 	array('onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			$tips 	= 	array('tips'=>$tip);
	...
	function password($nameid, $label='', $rules='', $value='', $atts=array(),$tip='',$att=array()) 
	{
		if ($tip!='') {
			$tips 	= 	array('onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			foreach ($att as $keys => $values) 
	...
	function textarea($nameid, $label='', $rules='', $value='', $atts=array(),$tip='',$att=array()) 
	{
		if ($tip!='') {
			$tips 	= 	array('onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			foreach ($att as $keys => $values) 
		...
		$info['rules'] = $rules;
		$info['value'] = $value;
		if (empty($info['cols'])) $info['cols'] = 50; // get this from config file
		if (empty($info['rows'])) $info['rows'] = 5;		
		$this->add($info);
		return $this;
	}	
	
	function upload($nameid, $label='', $required=FALSE, $atts=array(),$tip='',$attsl='') 
	{
	
        //$tip	=	$atts['upload_path'];
		//if ($attsl)
		if ($tip!='') {
			$tips 	= 	array('class'=>$attsl,'onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			$atts	=	array($atts,$tips);
		}	
		$info = $this->_make_info($atts);
		$this->_make_nameid($nameid, $info);
	...
	function iupload($nameid, $label='', $required=FALSE, $atts=array(),$tip='') 
	{
		$info = $this->_make_info($atts);
		if (empty($info['allowed_types'])) $info['allowed_types'] = 'jpg|png|gif';
		$this->upload($nameid, $label, $required, $info,$tip);
		return $this;
	}	
	
	function select($nameid, $options=array(), $label='', $selected='', $rules='', $atts=array(),$tip='',$att=array()) 
	{
		if ($tip!='') {
			//$tips 	= 	array('onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			//foreach ($att as $keys => $values) 
	...
	function checkbox($nameid, $value, $label='', $checked=FALSE, $rules='', $atts=array(),$tip='',$att=array()) 
	{
		if ($tip!='') {
			$tips 	= 	array('onmouseover'=>"showhint('".$tip."', this, event, '425px')");
			//foreach ($att as $keys => $values) 
